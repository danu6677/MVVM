...
final class Network: NSObject {
    ...
    //Network resquest types & Responce structures
      let NETWORK_REQUEST_TYPES = (G: "GET", P: "POST", U: "PUT", D: "DELETE")
      let RESPONSE_STRUCTURE = (C:"Codable_Object", O: "Other_Forms")
        ...
        let task = configurationManager.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            guard let data: Data = data, let response: URLResponse = response, error == nil else {
                ...
                if (error! as NSError).code == Constants.ERROR_CODE_INTERNET_OFFLINE || (error! as NSError).code == Constants.ERROR_CODE_NETWORK_CONNECTION_LOST {
                    ...
                    failureBlock(Constants.ERROR_CODE_INTERNET_OFFLINE, Constants.INTERNET_OFFLINE_MESSAGE, headers)
                }
                    ...
                    failureBlock(error_code, Constants.SERVER_ERROR_MESSAGE, headers)
                }
                
                else if (error! as NSError).code == Constants.STATUS_CODE_BAD_REQUEST {
                    ...
                    failureBlock(0, Constants.UNKNOWN_ERROR_MESSAGE, headers)
                }
                return
            }
                ...
                if responseStructure == self.RESPONSE_STRUCTURE.C {
                    ...
                    do {
                     ...
                     /* Error In Json response - De-serialization - Codable_Object */
                        failureBlock(0, Constants.RESPONSE_DE_SERIALIZATION_ERROR_MESSAGE, headers)
                    }
                }
                 else {
...
extension Network {
    ...
    func getFilms(successBlock: @escaping SuccessBlock, failureBlock: @escaping FailureBlock) {
        
        guard let url = URL(string: Config.getFilms) else {return}
        let responseModel = AllFilms.self
        let requestOptions: [String: String] = ["Content-Type": "application/json"]
        
        performWebServiceRequest(type: responseModel, with: url, requestType: NETWORK_REQUEST_TYPES.G, requestOptions: requestOptions, responseStructure: RESPONSE_STRUCTURE.C, successBlock: successBlock, failureBlock: failureBlock,return_response: responseModel)
        
    }
    
    func getCharacters(characterId:String,successBlock: @escaping SuccessBlock, failureBlock: @escaping FailureBlock) {
        guard let url = URL(string: Config.getCharacter+characterId) else {return}
        let responseModel = APICharacter.self
        let requestOptions: [String: String] = ["Content-Type": "application/json"]
        
        performWebServiceRequest(type: responseModel, with: url, requestType: NETWORK_REQUEST_TYPES.G, requestOptions: requestOptions, responseStructure: RESPONSE_STRUCTURE.C, successBlock: successBlock, failureBlock: failureBlock,return_response: responseModel)
    }
    func reachable(successBlock: @escaping SuccessBlock, failureBlock: @escaping FailureBlock) {
        guard let url = URL(string: Config.BASE_URL) else {return}
        let requestOptions: [String: String] = ["Content-Type": "application/json"]
        performWebServiceRequest(type: API.self, with: url, requestType: NETWORK_REQUEST_TYPES.G, requestOptions: requestOptions, responseStructure: RESPONSE_STRUCTURE.C, successBlock: successBlock, failureBlock: failureBlock,return_response: API.self)
    }
